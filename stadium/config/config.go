...
func Load() Config {
	...
	// type assertation
	config.App = cast.ToString(getOrReturnDefaultValue("PROJECT_NAME", "stadium"))
	...
	config.PostgresUser = cast.ToString(getOrReturnDefaultValue("POSTGRES_USER", "muhammadjonhalimov"))
	config.PostgresPassword = cast.ToString(getOrReturnDefaultValue("POSTGRES_PASSWORD", "00002222"))
	config.PostgresDatabase = cast.ToString(getOrReturnDefaultValue("POSTGRES_DATABASE", "stadium"))
